...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/DJPIPERHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
search_gifs = db.Table('search_gifs', db.Column('gifs', db.Integer, db.ForeignKey('Gif.id')), db.Column('searchterm', db.Integer, db.ForeignKey('SearchTerm.id')))
...
user_collection = db.Table('user_collection', db.Column('gifs', db.Integer, db.ForeignKey('Gif.id')), db.Column('collection_id', db.Integer, db.ForeignKey('PersonalGifCollection.id')))
...
def get_or_create_gif(title, url):
    ...
    gfs = Gif.query.filter_by(title=title).first()
    if gfs:
        return gfs
    else:
        gfs = Gif(title=title, embedURL=url)
        db.session.add(gfs)
        db.session.commit()
        return gfs
     # Replace with code
...
def get_or_create_search_term(term):
    ...
    s_term = SearchTerm.query.filter_by(term=term).first()
    if s_term:
        return s_term
    else:
        s_term = SearchTerm(term=term)
        findgifs = get_gifs_from_giphy(term)
        for g in findgifs:
            new_gif = get_or_create_gif(g['title'], g['embed_url'])
            s_term.gifs.append(new_gif)
        db.session.add(s_term)
        db.session.commit()
        return s_term
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    pgc = PersonalGifCollection.query.filter_by(name=name, user_id=current_user.id).first()
    if pgc:
        return pgc
    else:
        pgc = PersonalGifCollection(name=name, user_id=current_user.id)
        for g in gif_list:
            pgc.gifs.append(g)
        db.session.add(pgc)
        db.session.commit()
...
def create_collection():
    ...
    if request.method == 'POST':
        select_gifs = form.gif_picks.data
        gif_objs = [get_gif_by_id(int(id)) for id in select_gifs]
        get_or_create_collection(name=form.name.data, current_user=current_user, gif_list= gif_objs)
